...
def load_patient(data_root, patient_id):
    ...
    gt_mask = np.stack([left], axis=0)  # (2, D, H, W)
...
def run_inference(model, scan_tensor, device, model_name, gbp_class):
    """
    Returns:
        pred_masks:   (1, D, H, W) float32 numpy — predicted probabilities
    ...
    pred_masks = pred[0].cpu().numpy()  # (1, D, H, W)
    del pred
...
def save_results(pred_masks, saliency_l, out_dir, prefix):
...
if __name__ == "__main__":
    ...
    if args.model in ("basic", "both"):
        ...
        pred_masks, sal_l = run_inference(
            model, scan_tensor, device, "basic", GuidedBackprop
        )
        save_results(pred_masks, sal_l, out_dir, "basic")
        del model
        if device.type == "mps":
    ...
    if args.model in ("cbam", "both"):
        ...
        save_results(pred_masks, sal_l, out_dir, "cbam")
        del model
        if device.type == "mps":
            torch.mps.empty_cache()
